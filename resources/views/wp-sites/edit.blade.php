...
@section('header')
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-serif font-bold text-slate-900 tracking-tight">Edit WP Site: {{ $wpSite->site_name }}</h1>
            <p class="text-xs text-slate-500 mt-1">Perbarui koneksi WordPress dan penempatan perusahaan jika diperlukan.</p>
        </div>
    </div>
@endsection
...
<div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6"
     x-data="{
        ...
        selectedCategoryIds: @js(array_map('strval', old('category_ids', $wpSite->categories->pluck('id')->all()))),
    ...
    <form action="{{ route('wp-sites.update', $wpSite) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
                ...
                @if(auth()->user()->hasRole('super_admin'))
                    <div>
                        ...
                        <select id="company_id" name="company_id" x-model="selectedCompanyId" required class="w-full bg-white text-slate-800 text-xs border border-slate-200/80 rounded-xl px-3 py-2.5 focus:outline-none focus:border-[#C59B27] transition">
                            <option value="" disabled {{ old('company_id', $wpSite->company_id) ? '' : 'selected' }}>Pilih perusahaan...</option>
                            @foreach($companies as $company)
                                <option value="{{ $company->id }}" {{ old('company_id', $wpSite->company_id) == $company->id ? 'selected' : '' }}>{{ $company->name }}</option>
                            @endforeach
                        </select>
                    </div>
                @else
        ...
        <div class="pt-4 border-t border-slate-200 flex items-center justify-start gap-2">
            ...
            <button type="submit" class="inline-flex items-center gap-2 bg-[#C59B27] hover:bg-[#b08820] text-white text-xs font-semibold px-4 py-2.5 rounded-lg transition">Perbarui WP Site</button>
        </div>
    </form>
</div>
@endsection
