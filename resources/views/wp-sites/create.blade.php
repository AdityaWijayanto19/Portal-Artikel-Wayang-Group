...
@section('header')
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-serif font-bold text-slate-900 tracking-tight">Tambah WP Site</h1>
            <p class="text-xs text-slate-500 mt-1">Hubungkan target WordPress ke perusahaan tertentu untuk publikasi artikel.</p>
        </div>
    </div>
@endsection
...
<div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6"
     x-data="{
        ...
        selectedCategoryIds: @js(array_map('strval', old('category_ids', []))),
    ...
    <form action="{{ route('wp-sites.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div class="space-y-5">
                <div>
                    ...
                    <input id="site_name" name="site_name" type="text" value="{{ old('site_name') }}" required class="w-full bg-white text-slate-800 text-xs border {{ $errors->has('site_name') ? 'border-rose-300 focus:border-rose-500' : 'border-slate-200/80 focus:border-[#C59B27]' }} rounded-xl px-3 py-2.5 focus:outline-none transition">
                </div>

                <div>
                    <label for="site_url" class="block text-xs font-semibold text-slate-700 mb-1.5">URL Situs</label>
                    <input id="site_url" name="site_url" type="url" value="{{ old('site_url') }}" required class="w-full bg-white text-slate-800 text-xs border {{ $errors->has('site_url') ? 'border-rose-300 focus:border-rose-500' : 'border-slate-200/80 focus:border-[#C59B27]' }} rounded-xl px-3 py-2.5 focus:outline-none transition">
                </div>

                <div>
                    <label for="wp_username" class="block text-xs font-semibold text-slate-700 mb-1.5">Username WordPress</label>
                    <input id="wp_username" name="wp_username" type="text" value="{{ old('wp_username') }}" required class="w-full bg-white text-slate-800 text-xs border {{ $errors->has('wp_username') ? 'border-rose-300 focus:border-rose-500' : 'border-slate-200/80 focus:border-[#C59B27]' }} rounded-xl px-3 py-2.5 focus:outline-none transition">
                </div>

                <div>
                    <label for="wp_app_password" class="block text-xs font-semibold text-slate-700 mb-1.5">Application Password</label>
                    <input id="wp_app_password" name="wp_app_password" type="password" value="{{ old('wp_app_password') }}" required class="w-full bg-white text-slate-800 text-xs border {{ $errors->has('wp_app_password') ? 'border-rose-300 focus:border-rose-500' : 'border-slate-200/80 focus:border-[#C59B27]' }} rounded-xl px-3 py-2.5 focus:outline-none transition">
                </div>
            </div>

            <div class="bg-slate-50/70 p-5 rounded-2xl border border-slate-200/80 space-y-4">
                <h3 class="text-xs font-bold text-slate-800 uppercase tracking-wider">Penempatan Perusahaan</h3>

                @if(auth()->user()->hasRole('super_admin'))
                    <div>
                        <label for="company_id" class="block text-xs font-semibold text-slate-700 mb-1.5">Perusahaan / Tenant</label>
                        <select id="company_id" name="company_id" x-model="selectedCompanyId" required class="w-full bg-white text-slate-800 text-xs border border-slate-200/80 rounded-xl px-3 py-2.5 focus:outline-none focus:border-[#C59B27] transition">
        ...
        <div class="pt-4 border-t border-slate-200 flex items-center justify-start gap-2">
            ...
            <button type="submit" class="inline-flex items-center gap-2 bg-[#C59B27] hover:bg-[#b08820] text-white text-xs font-semibold px-4 py-2.5 rounded-lg transition">Simpan WP Site</button>
        </div>
    </form>
</div>
@endsection
