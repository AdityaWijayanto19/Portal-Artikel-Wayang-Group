@php
    /**
     * Partial form artikel (dipakai create & edit).
     *
     * Layout (hasil refactor "WordPress Sync Optimization"):
     *   KIRI  → fokus konten: Judul (jadi <h1> di WP) + Smart Editor (TinyMCE) +
     *           Accordion SEO (focus keyphrase, SEO title, slug, meta desc,
     *           Google SERP Preview, dan sub-seksi Analisis SEO).
     *   KANAN → meta publikasi & media: Gambar, Status, Author (sinkron WP),
     *           Kategori (terisolasi tenant), Situs WP, dan Tag freeform.
     *
     * Semua pilihan (kategori/situs/author) SUDAH terisolasi per-perusahaan dari controller.
     */
    ...
    $selectedStatus = old('status', $isEdit && in_array($article->status, ['published', 'queued']) ? 'published' : 'draft');
@endphp
...
<div x-data="articleEditor({
    ...
    <div class="lg:col-span-8 space-y-5">
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5 space-y-2">
            ...
            <p class="text-[10px] text-slate-400">Judul ini akan menjadi <strong>&lt;h1&gt;</strong> pada artikel WordPress.</p>
            @error('title')
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
            <x-rich-editor name="content" label="Isi Artikel" required
                :value="old('content', $isEdit ? $article->content : '')" />
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <button type="button" @click="seoOpen = !seoOpen"
                ...
                <span class="flex items-center gap-2">
                    ...
                    <span class="text-xs font-bold text-slate-700 uppercase tracking-wider">Optimasi SEO &amp; Google Preview</span>
            ...
            <div x-show="seoOpen" x-collapse class="px-5 pb-5 space-y-5 border-t border-slate-100 pt-5">
                ...
                <div class="space-y-1.5">
                    <label for="yoast_title" class="block text-xs font-bold text-slate-700 uppercase tracking-wider">SEO Title</label>
                    ...
                    <p class="text-[10px] text-slate-400">Kosongkan untuk memakai Judul Artikel. Panjang: <span x-text="(seoTitle || articleTitle).length"></span> karakter</p>
                </div>

                <div class="space-y-1.5">
                    <label for="slug_field" class="block text-xs font-bold text-slate-700 uppercase tracking-wider">Slug (URL)</label>
                    <input type="text" id="slug_field" x-model="slug" @input="slug = slugify(slug); slugEdited = true; recompute()"
                ...
                <div class="space-y-1.5">
                    <label for="yoast_metadesc" class="block text-xs font-bold text-slate-700 uppercase tracking-wider">Meta Description</label>
                ...
                <div class="space-y-2">
                    <div class="flex items-center justify-between">
                        <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider">Google Preview</label>
                    ...
                    <div class="border border-slate-200 rounded-xl p-4 bg-white"
                        ...
                        <div class="flex items-center gap-2 mb-1">
                            <span class="w-6 h-6 rounded-full bg-slate-100 flex items-center justify-center text-[9px] font-bold text-slate-500 shrink-0">W</span>
                            <div class="min-w-0">
                        ...
                        {{-- Judul biru --}}
                        <p class="text-[#1a0dab] text-lg leading-snug hover:underline cursor-pointer truncate" x-text="serpTitle"></p>
                        {{-- Deskripsi --}}
                ...
                <div class="border border-slate-200 rounded-xl overflow-hidden">
                    <button type="button" @click="analysisOpen = !analysisOpen"
                        ...
                        <span class="flex items-center gap-2">
                            <span class="text-[11px] font-bold text-slate-600 uppercase tracking-wider">Analisis SEO</span>
                            <span class="text-[10px] font-semibold px-2 py-0.5 rounded-full" :class="scoreBadgeClass" x-text="scoreCategory"></span>
                        </span>
                        <svg class="w-4 h-4 text-slate-400 transition-transform" :class="analysisOpen ? 'rotate-180' : ''"
                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                        </svg>
                    </button>
                    ...
                    <div x-show="analysisOpen" x-collapse class="p-4 space-y-4">
                        ...
                        <div class="flex items-center gap-4">
                            <div class="relative w-24 h-24 shrink-0">
                                <svg class="w-24 h-24 -rotate-90" viewBox="0 0 100 100">
                                    ...
                                    <circle cx="50" cy="50" r="42" fill="none" :stroke="scoreStroke" stroke-width="9"
                                        stroke-linecap="round" :stroke-dasharray="264"
                                        :stroke-dashoffset="264 - (264 * score / 100)"
                            ...
                            <div class="text-[11px] rounded-lg px-3 py-2 flex-1"
                                ...
                                <span x-show="score < 80">Skor minimal <strong>80</strong> untuk publish. Perbaiki indikator di bawah.</span>
                            </div>
                        </div>
                        ...
                        <div class="space-y-1 max-h-56 overflow-y-auto pr-1">
                            <template x-for="item in breakdown" :key="item.label">
                                <div class="flex items-center justify-between text-[11px] py-1 border-b border-slate-50">
                                    <div class="flex items-center gap-1.5 min-w-0">
                                        <svg class="w-3 h-3 shrink-0" :class="item.score >= item.max ? 'text-emerald-500' : (item.score > 0 ? 'text-amber-500' : 'text-slate-300')"
    ...
    <div class="lg:col-span-4 space-y-5 lg:sticky lg:top-4">
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5 space-y-3">
            ...
            <div class="space-y-1.5">
                ...
                <select name="status" id="status" x-model="status"
                    ...
                    <option value="published">Published (kirim ke WordPress)</option>
                </select>
            </div>
            ...
            <button type="submit"
                :disabled="status === 'published' && score < 80"
                :class="(status === 'published' && score < 80) ? 'opacity-50 cursor-not-allowed bg-slate-300' : (status === 'published' ? 'bg-[#C59B27] hover:bg-[#b08820] text-white' : 'bg-slate-800 hover:bg-slate-900 text-white')"
                ...
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 10l7-7m0 0l7 7m-7-7v18" x-show="status === 'published'" />
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" x-show="status !== 'published'" />
                </svg>
        ...
        {{-- Gambar Unggulan --}}
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5 space-y-3">
            <x-dropzone name="featured_image" label="Gambar Unggulan"
                :previewUrl="$isEdit && $article->featured_image_path ? asset('storage/' . $article->featured_image_path) : null" />
    ...
    <script>
        ...
        function articleEditor(initial) {
            return {
                ...
                get serpDesc() {
                    const d = (this.metadesc || 'Isi Meta Description untuk mengontrol cuplikan yang tampil di hasil pencarian Google.').trim();
                ...
                recompute() {
                    ...
                    parts.push({ label: 'SEO Title', max: 15, score: (tl >= 50 && tl <= 60) ? 15 : ((tl >= 35 && tl <= 75) ? 10 : (tl > 0 ? 5 : 0)) });

                    const ml = meta.length;
                    parts.push({ label: 'Meta Description', max: 10, score: (ml >= 120 && ml <= 156) ? 10 : ((ml >= 90 && ml <= 180) ? 6 : (ml > 0 ? 3 : 0)) });
                    ...
                    parts.push({ label: 'URL Slug', max: 10, score: Math.min(10, slugScore) });

                    parts.push({ label: 'Focus Keyword', max: 10, score: kw !== '' ? 10 : 0 });
                    parts.push({ label: 'Keyword di Title', max: 10, score: (kw && title.toLowerCase().includes(kw)) ? 10 : 0 });
                    parts.push({ label: 'Keyword di Heading', max: 10, score: (kw && lc.includes(kw)) ? 10 : 0 });

                    const kwCount = kw ? (lc.split(kw).length - 1) : 0;
                    const density = (kwCount * 100) / Math.max(1, words);
                    parts.push({ label: 'Keyword Density', max: 10, score: (density >= 1 && density <= 2.5) ? 10 : (density > 0 ? 6 : 0) });

                    const links = [...content.matchAll(/href=["']([^"']+)["']/gi)].map(m => m[1]);
                    parts.push({ label: 'Internal Link', max: 10, score: links.some(u => u.startsWith('/') || !u.startsWith('http')) ? 10 : 0 });
                    parts.push({ label: 'External Link', max: 5, score: links.some(u => u.startsWith('http')) ? 5 : 0 });
                    parts.push({ label: 'Alt Image', max: 5, score: alt !== '' ? 5 : 0 });
                    parts.push({ label: 'Content Length', max: 10, score: words >= 800 ? 10 : (words >= 500 ? 6 : (words > 0 ? 3 : 0)) });

                    const sentences = Math.max(1, (content.replace(/<[^>]+>/g, ' ').split(/[.!?]+/).filter(s => s.trim()).length) || 1);
                    const asl = words / sentences;
                    parts.push({ label: 'Readability', max: 5, score: asl <= 20 ? 5 : (asl <= 28 ? 3 : 1) });
