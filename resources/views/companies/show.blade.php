...
@section('header')
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <a href="{{ route('companies.edit', $company) }}" class="inline-flex items-center gap-1.5 bg-amber-50 hover:bg-amber-100 text-brand text-xs font-semibold px-3 py-2 rounded-lg border border-amber-200/60 transition">
...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
            ...
            <p class="text-2xl font-bold text-brand">{{ $company->articles_count }}</p>
        </div>
        <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5 space-y-4">
            ...
            <div class="flex items-center gap-3">
                <div class="w-12 h-12 rounded-lg border border-slate-200 bg-slate-50 flex items-center justify-center shrink-0 overflow-hidden">
                    ...
                    @else
                        <span class="text-sm font-bold text-brand">{{ strtoupper(substr($company->name, 0, 2)) }}</span>
                    @endif
                </div>
                <div>
