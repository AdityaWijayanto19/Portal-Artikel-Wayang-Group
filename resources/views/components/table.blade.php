...
<div id="{{ $containerId }}" class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
    ...
    @if ($searchAction || isset($filters) || $actions)
        <div
            class="p-4 sm:p-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white">

            @if ($searchAction)
                <form action="{{ $searchAction }}" method="GET" x-ref="filterForm"
                    class="flex flex-wrap sm:flex-nowrap items-center gap-3 flex-1 max-w-md">
                    ...
                    <div class="relative w-full" x-data="{
                        ...
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-400">
                            ...
                            <svg x-show="loading" class="animate-spin w-4 h-4 text-brand" fill="none"
                        ...
                        <!-- Input Search -->
                        <input type="text" name="{{ $searchName }}" x-model="search"
                            @input.debounce.400ms="performSearch()" placeholder="{{ $searchPlaceholder }}"
                            class="w-full pl-9 pr-4 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:outline-none focus:border-brand focus:ring-1 focus:ring-brand transition placeholder:text-slate-400 text-slate-700" />
                    </div>
            ...
            <thead class="bg-slate-50/70 border-b border-slate-100">
                <tr>
                    @foreach ($headers as $header)
                        ...
                        <th
                            class="px-6 py-4 text-xs text-slate-500 font-bold uppercase tracking-wider select-none {{ $alignClass }} {{ $width }}">
                            {{ $label }}
                        </th>
                    @endforeach
                </tr>
