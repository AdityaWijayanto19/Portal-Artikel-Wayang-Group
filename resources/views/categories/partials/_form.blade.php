    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-slate-50/70 p-5 rounded-2xl border border-slate-200/80 space-y-4">
            ...
            @if (auth()->user()->hasRole('super_admin'))
                <div>
                    <label for="company_id" class="block text-xs font-semibold text-slate-700 mb-1.5">Perusahaan /
                        Tenant</label>
                    <select id="company_id" name="company_id" required
                        class="w-full bg-white text-slate-800 text-xs border border-slate-200/80 rounded-xl px-3 py-2.5 focus:outline-none focus:border-[#C59B27] transition">
                        <option value="" disabled
                            {{ old('company_id', $category->company_id ?? '') ? '' : 'selected' }}>Pilih perusahaan...
                        </option>
